...
; Signature: string-starts-wth? String String -> boolean
; Purpose: the function will determine whether it follows the 3 situations described above

(define (string-starts-with? st1 st2)
  (cond
    [(string=? st1 st2) #true]
    [(string=? st2 "") #true]
    [(and (> (string-length st1) (string-length st2)) (string=? (substring st1 0 1) (substring st2 0 1))) #true] 
    [else #false]))
...
(define (draw-door DoorState)
  (cond
    [(string=? DoorState DS-CLOSED) DOOR-CLOSED]
    [(string=? DoorState DS-LOCKED) DOOR-LOCKED]
    [(string=? DoorState DS-OPEN) DOOR-OPEN])) 
