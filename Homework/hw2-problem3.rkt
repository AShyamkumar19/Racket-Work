...
; takes a simple word and outputs true and false
; Intrepretation: if sw matches SW-CORRECT or SW-INCORRECT its false

; Signature: simple-wordle-done? String -> boolean

(check-expect (simple-wordle-done? ":)") #true)
(check-expect (simple-wordle-done? ":(") #true)
(check-expect (simple-wordle-done? "dp") #false)
...
; Displays a letter in a box
; Interpretation: draws a box and the check-expect declares/execute the fucntion

(check-expect (draw-simple-wordle "A") (overlay
                                        (text "A" 30 "red")
                                         box))
(check-expect (draw-simple-wordle "B") (overlay
                                        (text "B" 30 "red")
                                         box))
(check-expect (draw-simple-wordle "C") (overlay
                                        (text "C" 30 "red")
                                         box))
(check-expect (draw-simple-wordle "D") (overlay
                                        (text "D" 30 "red")
                                         box))
(check-expect (draw-simple-wordle "E") (overlay
                                        (text "E" 30 "red")
                                         box))
(check-expect (draw-simple-wordle ":)") (overlay
                                        (text ":)" 30 "red")
                                         box))
(check-expect (draw-simple-wordle ":(") (overlay
                                        (text ":(" 30 "red")
...
; Signature: draw-simple-wordl String -> image

(define (draw-simple-wordle SimpleWordle)
  (overlay
   (text SimpleWordle 30 "red")
    box))
...
; Signature: key-simple-wordle Key choice -> image with letter
; Purpose: it will take the letter (make into uppercase) and return letter in box

(define (key-simple-wordle/empty ke)
  (cond
    [(string=? (string-upcase ke) "A") SW-A]
    [(string=? (string-upcase ke) "B") SW-B]
    [(string=? (string-upcase ke) "C") SW-C]
    [(string=? (string-upcase ke) "D") SW-D]
    [(string=? (string-upcase ke) "E") SW-E]
    [else SW-EMPTY]))
; Signature: key-simple-wordle/letter String Key Choice -> deleting image or entering to next line
; Purpose: to enter to next line or delete previous image
